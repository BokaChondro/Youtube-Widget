/**
 * =========================================================
 *  YouTube Top Cards (Cyberpunk Engine)
 *  - Renders 3 KPI cards + HUD
 *  - Handles Sci-Fi Glitch effects on random counters
 *  - Refreshes every 60s
 * =========================================================
 */
...
const FEEDBACK = {
  subs: { red: "CRITICAL LEAK", orange: "SLOW UPLINK", yellow: "STABLE", green: "STRONG SIGNAL", blue: "SURGE DETECTED", purple: "GOD MODE" },
  views: { red: "OFFLINE", orange: "LOW BANDWIDTH", yellow: "STABLE", green: "HIGH TRAFFIC", blue: "VIRAL SPIKE", purple: "NETWORK SATURATION" },
  watch: { red: "NO RETENTION", orange: "SIGNAL LOSS", yellow: "CONSISTENT", green: "HIGH ENGAGE", blue: "ADDICTIVE", purple: "LEGENDARY" }
};
...
const GLITCH_TARGETS = [
  'subsNow', 'viewsTotal', 'watchNow', 
  'subsVsNum', 'viewsVsNum', 'watchVsNum', 
  'subsNextPct', 'viewsNextPct', 'watchNextPct'
];

function triggerRandomGlitch() {
  // Pick a random element
  const id = GLITCH_TARGETS[Math.floor(Math.random() * GLITCH_TARGETS.length)];
  const el = document.getElementById(id);
  if (!el) return;
  
  // Save original text if needed, though CSS attr() uses data-text
  const currentTxt = el.textContent;
  el.setAttribute('data-text', currentTxt);
  el.classList.add('glitch-effect');
  
  // Remove after short burst
  setTimeout(() => {
    el.classList.remove('glitch-effect');
  }, 300 + Math.random() * 200); // 300-500ms duration
}

// Start glitch loop (random intervals between 2s and 6s)
(function glitchLoop() {
...
function setMainArrow(elId, tier) {
  ...
  if (el) { el.textContent = tierArrow(tier); el.style.color = "var(--c-tier)"; }
}
...
function setVsRG(elNumId, elArrowId, delta, decimals = 0, suffix = "") {
  ...
  numEl.setAttribute('data-text', numEl.textContent); // For glitch
}
...
function ensureSparkGradient(svgEl, gradId, tierHex) {
  ...
  if (stops[0]) stops[0].setAttribute("stop-color", rgbaFromHex(tierHex, 0.4));
...
function renderPacing(elId, cur, prev, suffix = "") {
  ...
  let pctHtml = pct > 0 ? `<span style="color:var(--c-green); font-weight:700;">[+${pct}%]</span>` : (pct < 0 ? `<span style="color:var(--c-red); font-weight:700;">[${pct}%]</span>` : `<span style="color:#666;">[=]</span>`);
  const left = `<div><span style="opacity:0.5; margin-right:4px;">LAST 7D:</span><b>${fmt(c)}${suffix}</b> ${pctHtml}</div>`;
  const right = `<div><span style="opacity:0.3; margin-right:4px;">PREV:</span><span style="opacity:0.6">${fmt(p)}${suffix}</span></div>`;
...
function setRollInstant(el, text) {
  ...
  el.setAttribute('data-text', text); // Update glitch data
}
...
function animateCasinoRoll(el, fromVal, toVal, opts = {}) {
  ...
  // Update data-text after animation for glitch accuracy
  setTimeout(() => el.setAttribute('data-text', txt(b)), duration);
}
...
function animateSpeedometer(el, toVal, opts = {}) {
  ...
  const tick = (now) => {
    ...
    const curT = renderText(endVal * easeOutCubic(p));
    el.textContent = curT;
    el.setAttribute('data-text', curT); // Keep glitch data sync
...
function render(data, isFirst) {
  ...
  document.getElementById("updated").textContent = `NEURAL LINK ONLINE • ${nowStamp()}`;
...
async function load(isFirst) {
  ...
  catch (e) { document.getElementById("updated").textContent = "LINK ERROR: " + e.message; }
}
...
const HUD_CONFIG = {
  interval: 16000,
  timer: null, started: false, lastGlitch: 0,
...
const KB = {
  facts: [ "YouTube is the 2nd most visited site.", "Mobile users visit 2x more than desktop.", "Algorithm favors Watch Time over Views.", "Bright thumbnails have higher CTR." ],
  tips: [ "AUDIO IS KING.", "HOOK IN 5 SECONDS.", "USE END SCREENS.", "REPLY TO COMMENTS." ],
  motivation: [ "CONSISTENCY IS THE CHEAT CODE.", "FOCUS ON THE 1 VIEWER.", "DON'T STOP UPLOADING." ]
};
...
function buildIntel(data) {
  ...
  if (uploadDaysAgo !== null && uploadDaysAgo > 14) out.push({ key: "warn_gap", cat: "warning", weight: 3, icon: HUD_ICONS.warn, tag: "ALERT", type: "red", text: `BUFFER EMPTY. LAST UPLOAD ${uploadDaysAgo}D AGO.` });
  else if (uploadDaysAgo !== null && uploadDaysAgo <= 3) out.push({ key: "good_gap", cat: "good", weight: 2, icon: HUD_ICONS.up, tag: "ACTIVE", type: "green", text: `CONSISTENCY DETECTED. ${uploadDaysAgo}D SINCE UPLOAD.` });

  if (weekG > 0 || weekL > 0) out.push({ key: "churn", cat: "subs", weight: 3.2, icon: weekL>weekG?HUD_ICONS.down:HUD_ICONS.up, tag: weekL>weekG?"LOSS":"GAIN", type: weekL>weekG?"red":"green", text: `NET: ${weekNet}. +${weekG} / -${weekL}.` });

  const thumb = hud.thumb28;
  if (thumb && thumb.ctr) {
    const ctr = thumb.ctr;
    out.push({ key: "ctr", cat: "packaging", weight: 2.1, icon: ctr<2?HUD_ICONS.warn:HUD_ICONS.bulb, tag: ctr<2?"LOW CTR":"CTR", type: ctr<2?"red":(ctr>8?"green":"yellow"), text: `CTR AVG: ${ctr.toFixed(1)}%. ${ctr<2?"OPTIMIZE THUMBNAILS.":"HEALTHY."}` });
  }

  const lv = hud.latestVideo;
  if (lv && lv.title) {
    out.push({ key: "lv_stat", cat: "video", weight: 2.8, icon: HUD_ICONS.rocket, tag: "LATEST", type: "purple", text: `"${lv.title.toUpperCase()}" — ${fmt(lv.views)} VIEWS.` });
  }
...
function showNextIntel() {
  ...
  setTimeout(() => {
    ...
    // Trigger Glitch if cooldown passed (8s)
    const now = Date.now();
    if (now - HUD_CONFIG.lastGlitch > 8000) {
      if (Math.random() > 0.3) {
        box.classList.add("glitch-active");
        HUD_CONFIG.lastGlitch = now;
      }
    }
  }, 200);
}
