...
/* =========================================================
   GLITCH EFFECT SYSTEM — Cyberpunk Digital Distortion
   ---------------------------------------------------------
       Randomly triggers glitch animations on counters and text elements.
       Creates that authentic sci-fi data corruption aesthetic.
   ========================================================= */
...
function triggerGlitchShadow(elementId) {
  const el = document.getElementById(elementId);
  if (!el) return;
  
  el.classList.add('glitch-active-shadow');
  setTimeout(() => {
    el.classList.remove('glitch-active-shadow');
  }, 400);
}

// Random glitch scheduler for counters
function scheduleRandomGlitches() {
  const counterIds = ['subsNow', 'rtNow', 'viewsTotal', 'watchNow'];
  const metaIds = [
  ...
  // Glitch a random counter every 8-15 seconds
  setInterval(() => {
    if (Math.random() > 0.3) { // 70% chance
      const randomCounter = counterIds[Math.floor(Math.random() * counterIds.length)];
      triggerGlitchShadow(randomCounter);
    }
  }, 8000 + Math.random() * 7000);
  
  // Glitch random meta values every 12-20 seconds  
  setInterval(() => {
    if (Math.random() > 0.4) { // 60% chance
      const randomMeta = metaIds[Math.floor(Math.random() * metaIds.length)];
      triggerGlitch(randomMeta);
    }
  }, 12000 + Math.random() * 8000);
  
  // Occasionally glitch the HUD message (less frequent)
  setInterval(() => {
    if (Math.random() > 0.6) { // 40% chance
      const msg = document.getElementById("hudMessage");
      if (msg) {
        msg.classList.add('glitch-active');
        setTimeout(() => msg.classList.remove('glitch-active'), 300);
      }
    }
...
(async function init() { 
  ...
  // Start glitch effects after initial load
  setTimeout(() => scheduleRandomGlitches(), 3000);
})();
